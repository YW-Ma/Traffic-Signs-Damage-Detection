...
%%%%%%%% NOTICE: Sometimes, you need to re-cluster multiple times to get
%%%%%%%% the result you want!!!!!!!

% part 2: for station 1
clear
for i = 25:25
    load listname
    load bbox
    ...
    %3: display & crop
    img = imread(['Station1\',listname{i},'.JPG']);
    ...
    for j=1:length(ID)
        ...
        if x<=right && x>=left && y<=bottom && y>=top
            cnt = cnt+1;
            container(cnt,:)=[j,Xi, Yi, Zi,INTENSITY(j)];
            color = COLOR(i,:);
            for c = 1:3
                img(y,x,c) = color(c);
            end
    ...
    [idx,C,sumd,D] = kmeans(container(:,7),cls);
    container(:,6) = idx;
